...
6. DB연결
    ...
    6. DB데이터어댑터(`SqlDataAdapter`) : DB연결이후 데이터처리를 쉽게 도와주는 객체
        - DB명령객체처럼 쿼리를 직접실행할 필요 없음
...
- MVC : Model View Controller 패턴
    - 사용자 인터페이스(View)와 비즈니스 로직(Controller, Model)을 분리해서 앱을 개발
    - 디자이너에게 개발에 일부를 참여를 시킴
    ...
    - Spring Boot, `ASP.NET`, jDango 등 웹개발 아키텍쳐패턴을 표준으로 사용
...
- 조금 쉽게 개발하고자 3rd Party에서 개발한 MVVM Framework를 사용
- 죵류
...
8. Bootstrapper 클래스를 생성, 작성 - [소스](./day02/Day02Wpf/WpfBasicApp01/Bootstrapper.cs)
...
### Log 라이브러리 사용
- 개발한 앱, 솔루션의 현재상태를 계속 모니터링 하는 기술
- Log 사용법
    - 직접코딩방식
    - 로그 라이브러리 사용방식
- Log 라이브러리
    - **NLog** : 가볍고 쉽다, 빠르다, 데스크톱
    - Serilog : 어려운 편, 빠르다
...
https://github.com/user-attachments/assets/0fe51d30-a1f7-4e1a-94f5-26e9d112feb6
...
2. NuGet 패키지 사용할 기본 라이브러리 설치
    ...
    - MySql.Data
    - NLog
3. 폴더 생성 : Models, Views, ViewModels, Helpers
4. MVVM 구조 초기작업
5. UI구현
...
7. 데이터그리드 더블클릭해서 
    ...
    - xmlns:i="http://schemas.microsoft.com/xaml/behaviors" 추가
...
https://github.com/user-attachments/assets/66b2fc14-f0af-496b-8624-cb1130ec1226
...
4. 위도(Latitude/Lat), 경도(Longittude/Lng) 표현



https://github.com/user-attachments/assets/a112106d-764c-4f24-ad9b-455132dcef7c
...
- 전면부
    ...
    <img src="./image/wpf0021.jpg" width="600">

- 후면부

    <img src="./image/wpf0022.jpg" width="600">

- [기존개발링크](https://github.com/hugoMGSung/hungout-with-arduino/tree/main/SmartHomeDIY)
...
- Server(MQTT 브로커) / Client(MQTT 소비자) 프로그램으로 동작
...
2. Mosquitto 설정파일 수정
    ...
    - #allow_anonymous false -> allow_anonymous true로 변경
    - 파일 저장 후, 서비스 재시작
3. Window 보안
    - 방화벽 및 네트워크 보호 > 고급 설정
    - 인바운드 규칙 > 새 규칙
    - 포트 선택 > 다음
    - TCP 선택, 특정포트 1883입력 > 연결 허용
4. MQTT Explorer 설치
...
https://github.com/user-attachments/assets/dbbb805d-315b-41d6-bb8d-ad3cdd05a15a
...
5. WPF MVVM 전체구현
6. MqttPub.py 와 Publish된 IoT데이터 WPF에서 Subscribe 예제



https://github.com/user-attachments/assets/2fdccb3e-8042-4f03-937e-5fed154b42cf
